(() => {
  ...
  function prepare(video) {
    ...
    video.loop = states.get(video)?.looping === true;
  ...
  function setActive(video, active, looping = false) {
    const state = states.get(video);
    if (!state || (state.active === active && state.looping === looping)) return;
    if (looping && !state.active && active) {
      state.userPaused = false;
      state.completed = false;
    }
