(()=>{
  ...
  function lockOnce() {
    if (reduceMotion || !nearHeroStart() || window.location.hash) return;
    try {
      const key = 'desorden:hero-intro-seen';
      if (sessionStorage.getItem(key)) return;
      sessionStorage.setItem(key, '1');
    } catch {
      // Without session storage, skip the lock rather than repeat it on return.
      return;
    }
    previousOverflow = root.style.overflow;
    root.style.overflow = 'hidden';
    unlockTimer = setTimeout(unlock, 2000);
  }
  ...
  function start() {
    ...
    try {
      ...
      playback?.catch?.(() => { started = false; unlock(); });
    } catch {
      started = false;
      unlock();
    }
  ...
  addEventListener('pagehide', unlock);
  if (mobile) {
