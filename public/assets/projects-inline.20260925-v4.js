(() => {
  ...
  async function loadPanel(panel, href, includeMedia) {
    ...
    const mobileStack = card.closest('[data-project-stack]') ? matchMedia('(max-width:699px)') : null;
    if (mobileStack) {
      const syncLink = () => {
        setExpanded(card, trigger, panel, false);
        if (mobileStack.matches) {
          ['role', 'aria-controls', 'aria-expanded'].forEach(name => trigger.removeAttribute(name));
          if (marker) marker.textContent = ' ›';
        } else {
          trigger.setAttribute('role', 'button');
          trigger.setAttribute('aria-controls', panel.id);
      ...
      mobileStack.addEventListener('change', syncLink);
      syncLink();
    }
