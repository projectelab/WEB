...
test('PageSpeed CSS delivery keeps only critical CSS render-blocking',async()=>{
 for(const route of routes){
  const html=await read(`public${route}index.html`);
  assert.match(html,/site\.20260925-v4\.css/,`${route}: critical site CSS`);
  assert.match(html,/anton-latin\.20260925\.woff2/,`${route}: local Anton preload`);
  ...
  for(const link of extras){
   assert.match(link[0],/media="print"/,`${route}: noncritical CSS media`);
   assert.match(link[0],/data-noncritical-css/,`${route}: noncritical CSS marker`);
   assert.match(html,/noncritical-css\.20260925-v1\.js/,`${route}: noncritical loader`);
  }
 }
});
